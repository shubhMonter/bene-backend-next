...
router.get("/combos/add", async function (req, res) {
  ...
  let productmeta = await ProductMeta.find()
    .select("-1")
    .sort({ _id: "desc" })
    .populate("productid")
    .populate("categoryid");
...
router.get("/combos/addCombo", async function (req, res) {
  // let productList = await ProductMeta.find({}).then((newRes) => console.log({newRes}))
  // console.log({productList})
  // res.render("add_combos.hbs", { productList })
  ...
  return res.status(200).json({ productList, products, categories });
});
router.post("/combos/add", upload.any(), async function (req, res) {
  ...
  let products =
    typeof req.body.select_product_combo === "string"
      ? req.body.select_product_combo.split(",")
      : req.body.select_product_combo;
  ...
  let weight = 0,
    ...
    width = 0,
    height = 0;
  let addInternally = products.map((el) => {
    ProductMeta.findById(el).then((data) => addthem(data));
    function addthem(data) {
      console.log({ "Add Them": data });
      weight = data.weight ? weight + data.weight : weight;
      servings = servings + parseInt(data.servings);
      servingsize = servingsize + parseInt(data.servingsize);
      totalcbdmg = data.totalcbdmg ? totalcbdmg + data.totalcbdmg : totalcbdmg;
      cbdperunitmg = data.cbdperunitmg
        ? cbdperunitmg + data.cbdperunitmg
        : cbdperunitmg;
      length = data.shipping_length ? length + data.shipping_length : length;
  ...
  setTimeout(function () {
    const comboProducts = Object.keys(req.body)
      .map((el) => {
        ...
        if (variationId) isVariable = true;

        if (combo_pid) return { combo_pid, sku, isVariable };
    ...
    if (req.body.reactCombo === "true") {
      combo
        .save()
        .then((combo) => res.status(200).json({ combo }))
        .catch((error) => {
          console.log(error);
          res.status(404).json({ message: "Combo Added Successfully" });
        });
    } else {
      combo
        .save()
        .then((combo) => res.redirect("/products/combos/all"))
  ...
  if (errors) {
    ...
    let addInternally = products.map((el) => {
      ...
      function addthem(data) {
        ...
        width = data.shipping_width ? width + data.shipping_width : width;
        height = data.shipping_height ? height + data.shipping_height : height;
        // console.log(weight);
        // console.log(servings);
        // console.log(servingsize);
        // console.log(totalcbdmg);
    ...
    setTimeout(function () {
      ...
      //     const matchedEl = changeGallery.find(
      //       (elx) => elx.elemMatch === index
      //     );
      ...
      // }
      if (typeof req.files !== "undefined" && req.files.length > 0) {
        combo.galleryimgdetails = combo_data.galleryimgdetails.concat(
          galleryArray
        );
      }
      ...
      console.log({ combo });
      if (req.body.reactCombo === "true") {
        Combos.findOneAndUpdate({ _id: id }, combo, { new: true })
          .then((combo) => res.status(200).json({ combo }))
          .catch((error) => {
            console.log(error);
            res.status(404).json({ message: "Combo Updated Successfully" });
          });
      } else {
        Combos.findOneAndUpdate({ _id: id }, combo)
          .then((combo) => res.redirect("/products/combos/all"))
...
router.post("/test-schema9", upload.any(), (req, res) => {
  console.log(req.body);
  console.log(req.files);
});
...
router.post("/add-product", upload.any(), async (req, res, next) => {
  ...
  let productExist = await Product.findOne({ sku: req.body.sku })
    .select("-1")
    .sort({ _id: "asc" });
  ...
  if (req.body.producttype === "variable") {
    ...
    var ccFlag = [...variationSkuArr, currentSku].some((el) =>
      skuList.includes(el)
    );
    // console.log({
    //     variationSkuArr,
  ...
  if (productExist || ccFlag) {
    return res
      .status(404)
      .json({ success: false, serialerror: "Duplicate Product Sku" });
  } else {
    function saveAttributes(array) {
      const returnVal = array.map((el) => {
        return {
          ...el,
          values:
            el.values.constructor === Array ? [...el.values] : [el.values],
        };
      });
    ...
    let skuval = (product.producttitle = req.body.producttitle);
    ...
    product
      .save()
      .then((product) => product_service(product))
      .catch((err) =>
        res.status(404).json({
          success: false,
    ...
    function product_service(product) {
      if (product) {
        ...
        if (req.body.manage_stock) {
          productMeta.managestockstatus =
            typeof req.body.manage_stock === "string"
              ? req.body.manage_stock === "true"
                ? true
                : false
              : req.body.manage_stock;
          productMeta.unit = req.body.stock;
        }
        if (req.body.enable_review) {
          productMeta.enablereview =
            typeof req.body.enable_review === "string"
              ? req.body.enable_review === "true"
                ? true
                : false
...
router.get("/edit/:id", async function (req, res) {
  let productmeta = await ProductMeta.findById(req.params.id)
    .select("-1")
    .sort({ _id: "desc" })
    .populate("productid")
    .populate("categoryid")
...
router.post("/edit/:id", upload.any(), async function (req, res) {
  ...
  if (errors) {
    ...
    productMeta.fieldname = req.body.fieldname;
    productMeta.fieldvalue = req.body.fieldvalue;
    productMeta.visibilitytype = req.body.visibilitytype || req.body.hide;
    productMeta.featured = req.body.featured;
    productMeta.packagingtype = req.body.packagetype || req.body.packagingtype;
    productMeta.vendorid = req.body.vendorid;
    productMeta.asin = req.body.asin;
    productMeta.use = req.body.use;
    productMeta.weight = req.body.weight;
    ...
    if (typeof req.files !== "undefined" && req.files.length > 0) {
      if (galleryArray.length > 0) {
        ...
        console.log({ galleryArray });
        productMeta.galleryimgdetails = productmeta_data.galleryimgdetails.concat(
          galleryArray
        );
    ...
    var data = {};
    try {
      data = await ProductMeta.findOneAndUpdate(
        { _id: req.params.id },
        productMeta,
        { new: true }
      );
...
router.get("/combos/getAll", async function (req, res) {
  Combos.find()
    .select("-1")
    .sort({ _id: "desc" })
    .populate("categoryid")
    .then((result) => res.status(200).json({ combos: result }))
    .catch((err) => res.status(404).json({ message: err.message }));
});
...
router.post("/api/bycategory", async function (req, res) {
  await Category.find({ categorytitle: req.body.title }).then((data) =>
    displayData(data)
  );
  async function displayData(data) {
    let id = data[0]._id;
    await ProductMeta.find({ categoryid: id })
      .select("-1")
      .sort({ _id: "-1" })
      .populate("productid")
      .populate("çategoryid")
      .then((data) => res.status(200).json({ success: true, products: data }));
  }
});
...
router.get("/api/combos/all", (req, res) => {
  Combos.find()
    ...
    .then((data) => {
      //res.setHeader('Cache-Control', 'Vary: Accept-Encoding Header, max-age=31557600');
      res.status(200).json({ success: true, combos: data });
    })
...
router.post("/combos/removeimage", upload.any(), async function (req, res) {
  ...
  if (req.body.action == "f_remove_combo") {
    ...
    Combos.update(query, combo, { new: true }, function (err, combo) {
      if (err) {
        res.status(404).json({
          success: false,
...
router.post("/removeimage", upload.any(), async function (req, res) {
  ...
  if (req.body.action == "f_remove") {
    ...
    productmeta.galleryimgdetails = galleryArray;
    let query = { _id: req.body.productid };
    ProductMeta.update(query, productmeta, { new: true }, function (err, data) {
      if (err) {
        res.json({
          success: false,
          message: "Error in Removing Image",
...
router.post("/combos/removebimage", upload.any(), async function (req, res) {
  ...
  combo.sectionbimage = "";
  let query = { _id: req.body.productid };
  Combos.findOneAndUpdate(query, combo, { new: true }, function (err) {
    if (err) {
      res.json({
        success: false,
        message: "Error in Removing Image",
...
router.delete("/delete/:id", async function (req, res) {
  ...
  let productmeta = await ProductMeta.findById(req.params.id).populate(
    "productid"
  );
...
router.get("/getAttribute", async function (req, res) {
  Attribute.find()
    .then((response) => res.status(200).json({ attribute: response }))
    .catch((err) => res.status(404).json({ message: err.message }));
});
...
router.post("/add-attribute", async (req, res) => {
  ...
  if (errors)
    return res
      .status(404)
      .json({ success: false, message: "Validation Error", errors: errors });
  let nameCheck = await Attribute.findOne({
    name: req.body.attributetitle,
  }).exec();
  if (nameCheck)
  ...
  if (attributeSave) {
    ...
    res.status(200).json({
      success: false,
      message: "Error in Saving the Attribute",
      test: attributeSave,
    });
  }
});
...
router.get("/attribute-data/:id", ensureAuthenticated, function (req, res) {
  Attribute.findById(req.params.id)
    .then((result) => attributedata(result))
    .catch((err) =>
      res
        .status(404)
...
router.post("/attribute-data/:id", upload.any(), async (req, res) => {
  ...
  let query = { _id: req.params.id };
  Attribute.findOneAndUpdate(query, attribute, { new: true })
    .then((response) =>
      res.status(200).json({
        success: true,
  ...
  Attribute.findById(req.params.id, function (err, attribute) {
    Attribute.remove(query, function (err) {
      if (err) {
        res.status(404).json({
          success: false,
          message: "Error in Deleteing Attribute",
          errors: err,
        });
      }
...
router.post("/attribute-term-delete", async function (req, res) {
  ...
  ProductMeta.find({ attributes: { $elemMatch: { names: attrislug } } }).exec(
    function (err, productmeta) {
  ...
  Attribute.update(query, attributes, function (err) {
    if (err) {
      return res.status(404).json({
        success: false,
        message: "Error in Adding Terms",
        errors: err,
      });
    } else {
      return res
        .status(200)
        .json({ success: true, message: "Terms Delete Successfully" });
    }
  });
});
...
router.post("/attribute-term/add", async (req, res) => {
  ...
  let attributeDetails = await Attribute.findById(req.body.attributeid).select(
    "-1"
  );
  let termsObject = attributeDetails.terms;
  ...
  Attribute.update(query, attributes, function (err) {
    if (err) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Terms Added Successfully" });
    }
  });
});
...
router.get("/api/getbyid/:id", async function (req, res) {
  ...
  if (productmeta) {
    ...
    if (combo) {
      ...
      return res
        .status(400)
        .json({ status: false, error: "No such product with this ID" });
    }
  }
});
...
router.get("/all-review", ensureAuthenticated, async function (req, res) {
  Product.find()
    .then((products) => all_products(products))
...
router.post("/add-review", async (req, res) => {
  ...
  review
    .save()
    .then((reviews) => save_product_data(reviews))
    .catch((err) => res.json({ status: false, error: err }));
...
router.post("/reviews-status-update", async function (req, res) {
  ...
  Review.findOneAndUpdate(query, review, function (err) {
    if (err) {
      return res.status(404).json({
        success: false,
        message: "Error in Updating Review Status",
        errors: err,
      });
    } else {
      return res
        .status(200)
        .json({ success: true, message: "Review Status Updated" });
    }
  });
});
